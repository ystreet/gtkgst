...
static void
gtk_gst_widget_get_preferred_width (GtkWidget * widget, gint *min, gint *natural)
{
...
static void
gtk_gst_widget_get_preferred_height (GtkWidget * widget, gint *min, gint *natural)
{
  GtkGstWidget *gst_widget = (GtkGstWidget *) widget;
...
gtk_gst_widget_draw (GtkWidget * widget, cairo_t *cr)
{
  ...
  if (gst_widget->priv->negotiated && gst_widget->priv->buffer
    ...
    gdouble scale_x = (gdouble) widget_width / GST_VIDEO_INFO_WIDTH (&frame.info);
    gdouble scale_y = (gdouble) widget_height / GST_VIDEO_INFO_HEIGHT (&frame.info);
